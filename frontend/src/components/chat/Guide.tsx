...
const Guide = () => {
  ...
  return (
    <div className="mb-2 mt-1 w-full flex whitespace-nowrap overflow-x-scroll no-scrollbar cursor-grab" {...events} ref={ref}>
      ...
      <GuideButton buttonName="내 주식" url={`/${userId}/stock`}  isIcon={true} icon={faUser} iconColor="text-stockBlue" />
      ...
      <GuideButton buttonName="내 거래" url={`/${userId}/trade`} isIcon={true} icon={faAddressBook} iconColor="text-[#c242f5]" />
      <GuideButton buttonName="내 파산" url={`/${userId}/bankrupt`} isIcon={true} icon={faHouseChimneyCrack} iconColor="text-black" />
    </div>
  );
};
